...
// int new_neighbor_cnt = 0;
// int current, next;

// #pragma omp parallel for reduction(+:new_neighbor_cnt)
//     for (int i=0; i<g->num_nodes; i++){
//         if(distances[i] == depth+1){
//             new_neighbor_cnt++;
//         }
//     }
//     new_frontier->count = new_neighbor_cnt;
...
//         if(distances[i] == depth+1){
//             do{
//                 current = new_neighbor_cnt;
//                 next = current + 1;
//             }while(!__sync_bool_compare_and_swap(&new_neighbor_cnt, current, next));
//             new_frontier->vertices[new_neighbor_cnt-1] = i;
//         }
//     }
    ...
    for (int i = 0; i < g->num_nodes; i++)
    {
        if(distances[i] == NOT_VISITED_MARKER)
        {
            ...
            for (int neighbor = start_edge; neighbor < end_edge; neighbor++)
            {
                ...
                if (frontier->vertices[incoming] != 0)
                {   
...
#pragma omp parallel for schedule(static)
    ...
    // setup frontier with the root node
    // make vertices as a tag, all vertices are in this list
    ...
    while (true)
    {
        ...
        if(!bottom_up_step(graph, frontier, new_frontier, sol->distances, depth)){
            break;
        }
...
#pragma omp parallel for schedule(static)
    ...
    while (frontier->count !=0)
    {
        ...
        if(frontier->count / graph->num_nodes < 0.2){
            top_down_step(graph, frontier, new_frontier, sol->distances);
        }
